...
export { mulberry32, openSimplex2SHeightmap, };
// =============================================================================
...
function grad(hash) {
    ...
    const cube = new Vec3(hash, hash / 2, hash / 4).floor().modAll(2).scale(2).subtractAll(1);
    // Random edge of the three edges connected to that vertex
    // Also a cuboctahedral vertex
    // And corresponds to the face of its dual, the rhombic dodecahedron
    const cuboct = cube.clone();
    cuboct.setIndex(Math.max(0, Math.floor(hash / 16.0)), 0.0);
    // In a funky way, pick one of the four points on the rhombic face
    const type = Math.floor(hash / 8.0) % 2.0;
    const rhomb = cube
        .clone()
        .scale(1 - type)
    ...
    const grad = cuboct.scale(1.22474487139).add(rhomb);
...
function openSimplex2SDerivativesPart(X) {
    const b = X.clone().floor();
    const i4 = new Vec4(X.x - b.x, X.y - b.y, X.z - b.z, 2.5);
    // Pick between each pair of oppposite corners in the cube.
    const v1 = b.clone().addAll(Math.floor(i4.dot(Vec4.all(0.25))));
    const v2 = b
        .clone()
        .add(Vec3.xAxis())
    ...
    let hashes = permute(new Vec4(v1.x, v2.x, v3.x, v4.x).modAll(289.0));
    ...
    const d1 = X.clone().subtract(v1);
    const d2 = X.clone().subtract(v2);
    const d3 = X.clone().subtract(v3);
    const d4 = X.subtract(v4);
    const a = Vec4.all(0.75)
        .subtract(new Vec4(d1.dot(d1), d2.dot(d2), d3.dot(d3), d4.dot(d4)))
        .max(0.0);
    const aa = a.clone().multiply(a);
    const aaaa = aa.clone().multiply(aa);
    const g1 = grad(hashes.x);
    const g2 = grad(hashes.y);
    const g3 = grad(hashes.z);
    const g4 = grad(hashes.w);
    const extrapolations = new Vec4(d1.dot(g1), d2.dot(g2), d3.dot(g3), d4.dot(g4));
...
const __openSimplex2OrthonormalMap = new Mat3(0.788675134594813, -0.211324865405187, -0.577350269189626, -0.211324865405187, 0.788675134594813, -0.577350269189626, 0.577350269189626, 0.577350269189626, 0.577350269189626);
...
function openSimplex2SHeightmap(position) {
    position.applyMat3(__openSimplex2OrthonormalMap);
    const result = openSimplex2SDerivativesPart(position.clone()).add(openSimplex2SDerivativesPart(position.addAll(144.5)));
    const result2 = result.xyz.applyMat3(__openSimplex2OrthonormalMap);
